...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		for (int i = 0; i < rows; i++) {
			for (int j = 0; j < cols; j++) {
				// Determine the column index of the flipped pixel
				// The pixel at column j moves to column (cols - 1 - j)
	...
	public static Color[][] flippedVertically(Color[][] image) {
		...
		for (int i = 0; i < rows; i++) {
			for (int j = 0; j < cols; j++) {
				// The pixel at row i moves to row (rows - 1 - i)
	...
	private static Color luminance(Color pixel) {
		...
		// Calculate weighted sum. We cast to int as color components are integers.
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		int h0 = image.length; // Original height
		int w0 = image[0].length; // Original width
		Color[][] newImage = new Color[height][width];

		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				// Map target pixel (i, j) back to source pixel
				// using the ratio of dimensions.
				int sourceY = i * h0 / height;
				int sourceX = j * w0 / width;
				newImage[i][j] = image[sourceY][sourceX];
			}
		}
		return newImage;
	}
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		// If target dimensions differ from source, scale target first
		if (target.length != rows || target[0].length != cols) {
			target = scaled(target, cols, rows);
		}

		for (int i = 0; i <= n; i++) {
			// Calculate alpha: Starts at 1 (all source) and goes to 0 (all target)
			// n - i goes from n to 0. So (n - i) / n goes from 1 to 0.
			double alpha = (double) (n - i) / n;

			Color[][] blendedImage = blend(source, target, alpha);
			display(blendedImage);
			StdDraw.pause(500); // Pause for 500ms
		}
	}
